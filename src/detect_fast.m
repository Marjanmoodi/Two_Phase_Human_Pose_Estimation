function [boxes,candids,pyra, unary_map, idpr_map] = detect_fast(iminfo, model, thresh, param)
...
tic
% load('saved_workspace3.mat');
iminfo.im = strrep(iminfo.im,'./dataset/LSP/images','./features');
file_path = strrep(iminfo.im,'jpg','mat');
% file_path = strrep(fi,'./dataset/LSP/images','./features');
try
   load(file_path);
% load('testFeat.mat');
catch
    % [pyra, unary_map, idpr_map] = imCNNdet(im,model,useGpu,1,param.impyra_fun);
    % save(file_name,'pyra','unary_map','idpr_map');
    error('There is no .mat file for this test image.please select another test!');
...
for level = levels,
    ...
    for c  = 1:length(model.components),
        ...
        if ~isempty(X)
            ...
            [box,candid] = backtrack(X,Y,parts,pyra(level));
            ...
            candid(:,3,:) = level;
            candids{level} = candid;
        end
    end
end
boxes = cat(1, boxes{:});
candids = cat(1,candids{:});
% Backtrack through DP msgs to collect ptrs to part locations
function [box,candid] = backtrack(x,y,parts,pyra)
numx     = length(x);
